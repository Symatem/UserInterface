    ...
    blocks: [
        {
            ...
            width: 7, height: 6,
            ports: [
                {x: 7, y: 5, direction: 0, label: 'Happen'}
            ]
        },
        {
            x: 4, y: 2,
            width: 2, height: 2,
            ports: [
                {x: 2, y: 1, direction: 0, label: 'Things'}
            ]
        },
        {
            x: 4, y: 2,
...
function renderBlock(block) {
    ...
    for (const port of block.ports) {
        ...
        let angle = port.direction * 90;
        let xOffset = (block.x + port.x) * gridSize;
        let yOffset = (block.y + port.y) * gridSize;
...
function renderConnection(connection) {
    const sourceX = connection.sourceBlock.x + connection.sourcePort.x;
    const sourceY = connection.sourceBlock.y + connection.sourcePort.y;
    const destinationX = connection.destinationBlock.x + connection.destinationPort.x;
    const destinationY = connection.destinationBlock.y + connection.destinationPort.y;
    const arrow = {
        x: sourceX - 1, y: sourceY,
        segments: [],
        slopes: [
            {direction: connection.sourcePort.direction, x: sourceX, y: sourceY},
            {direction: connection.destinationPort.direction, x: destinationX, y: destinationY}
        ]
    };

    arrow.segments.push({direction: 0, distance: 3});
    arrow.segments.push({direction: 1, distance: destinationY - sourceY - 1});
    arrow.segments.push({direction: 0, distance: 3});
