...
const renderLoweringOrLid = function (unit, isLid) {
    ...
    path.setAttribute('x', unit.rectangle.x * gridSize);
    path.setAttribute('y', unit.rectangle.y * gridSize);
...
var renderWire = function (wire, unit) {
    ...
    const sourceX = sourcePosition.x + ((wire.sourceSocket.type === socketType.in) ? .3 : .3);
    const destinationPosition = socketPosition(wire.destinationUnit, wire.destinationSocket);
    const destinationX = destinationPosition.x + ((wire.destinationSocket.type === socketType.in) ? -.5 : 0);
    const arrow = {
        x: sourceX, y: sourcePosition.y,
        segments: [
            {direction: directions.right, distance: destinationX - sourceX}
        ],
