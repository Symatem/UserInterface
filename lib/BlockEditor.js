...
const exampleGraph = function () {
    ...
    return {
        blocks: [outerBlock, innerBlock],
        connections: [{
            sourceBlock: innerBlock,
            destinationBlock: outerBlock,
            sourcePort: innerBlock.ports[0],
            destinationPort: outerBlock.ports[0]
        }]
    };
};
...
function renderBlockOrLid(block, isLid) {
...
var renderBlock = function (block) {
    renderBlockOrLid(block);
    if (block.hasLid) {
        renderBlockOrLid(block, true)
    }
};
    ...
    for (const connection of graph.connections) {
        ...
        const arrow = {
            ...
            slopes: [
                ...
                {
                    direction: connection.destinationPort.direction,
