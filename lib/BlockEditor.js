...
const exampleUnit = function (processUnitType = unitType.openOnHover) {
    const upperInnerY = 2;
    const lowerInnerY = 6;

    const read = (output_label, y)=> unit(
        'read_' + output_label,
        rectangle(1, y, 3, 2),
        [
            output(output_label)
    ...
    const readA = read('a', upperInnerY);
    const readB = read('b', lowerInnerY);
...
var renderWire = function (wire, unit) {
    ...
    if(wire.sourceUnit.type === unitType.open && wire.sourceSocket.type === socketType.in)
        arrow.slopes.push({direction: wire.sourceSocket.direction, position: sourcePosition});
    if(wire.destinationUnit.type === unitType.open && wire.destinationSocket.type === socketType.out)
        arrow.slopes.push({direction: wire.destinationSocket.direction, position: destinationPosition});

    arrow.segments.push({direction: directions.right, distance: destinationPosition.x - sourcePosition.x});

    renderArrow(arrow);
};

const renderSubUnits = function (unit) {
    for (const subUnit of unit.units)
        renderUnit(subUnit);
};
const renderWires = function (unit) {
    for (const wire of unit.wires)
        renderWire(wire, unit);
};
var renderSubComponents = function (unit) {
    renderSubUnits(unit);
    renderWires(unit);
};
const renderUnit = function (unit) {
    ...
    if (unit.type !== unitType.closed) {
        renderSubComponents(unit);
    }
...
const renderExample = function (processUnitType = unitType.openOnHover) {
    renderUnit(exampleUnit(processUnitType));
};

renderExample(unitType.open);