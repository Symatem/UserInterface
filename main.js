...
import { Utils, SymbolInternals, JavaScriptBackend } from '../SymatemJS/SymatemJS.mjs';
...
for(const name of Object.getOwnPropertyNames(backend.symbolByName))
    labelOfSymbol(backend.symbolByName[name], true);
...
function encodeHTML(element, dataValue) {
    if(dataValue instanceof Array) {
        ...
        label.innerText = Utils.encodeText(dataValue);
...
function decodeHTML(element) {
    if(element.children[0].tagName === 'SPAN')
        return Utils.decodeText(element.innerText);
    const dataValue = [];
...
function labelOfSymbol(symbol, forceUpdate) {
    ...
    if(!entry.label || forceUpdate) {
        ...
        if(data != undefined) {
            entry.label = Utils.encodeText(data);
            labelIndex.add(entry);
        } else {
            const namespaceSymbol = backend.symbolInNamespace('Namespaces', SymbolInternals.namespaceOfSymbol(symbol)),
                  namespaceData = backend.getData(namespaceSymbol);
            if(namespaceData)
                entry.label = Utils.encodeText(namespaceData)+':'+SymbolInternals.identityOfSymbol(symbol);
            else
                entry.label = Utils.encodeText(symbol);
        }
...
function addPanel(nodesToAdd, symbol) {
    ...
    namespaceSocket.symbol = backend.symbolInNamespace('Namespaces', SymbolInternals.namespaceOfSymbol(symbol));
    ...
    for(const triple of backend.queryTriples(backend.queryMasks.MVV, [panel.symbol, 0, 0]))
...
function findLabelsOfComposite(composite, labelsToUpdate) {
    for(const triple of backend.queryTriples(backend.queryMasks.VMM, [0, backend.symbolByName.Encoding, composite]))
        labelsToUpdate.add(triple[0]);
    for(const triple of backend.queryTriples(backend.queryMasks.VMM, [0, backend.symbolByName.Default, composite]))
...
function replaceTripleTemplate(socket, prevSymbol, nextSymbol, forward) {
    ...
    if(prevTriple[1] === backend.symbolByName.Encoding || nextTriple[1] === backend.symbolByName.Encoding)
...
function openSearch(socket) {
    ...
    function accept() {
        ...
        if(entry.symbol == undefined) {
            ...
            const encoding = backend.getPairOptionally(entry.symbol, backend.symbolByName.Encoding);
            if(encoding !== backend.symbolByName.Void)
                update.triples.push([entry.symbol, backend.symbolByName.Encoding, encoding]);
        }
    ...
    searchBar.onkeyup = function(event) {
        ...
        if(split.length === 2) {
            ...
            if(namespace && SymbolInternals.namespaceOfSymbol(namespace.entry.symbol) === SymbolInternals.identityOfSymbol(backend.symbolByName.Namespaces)) {
                const entry = {};
                if(namespace.entry.symbol !== backend.symbolByName.Index) {
                    entry.label = `Create in ${namespace.entry.label}`;
                    entry.namespace = SymbolInternals.identityOfSymbol(namespace.entry.symbol);
                    entry.data = Utils.decodeText(split[1]);
                } else {
                    const index = parseInt(split[1]);
                    if(!isNaN(index)) {
                        entry.label = `Index:${index}`;
                        entry.symbol = backend.symbolInNamespace('Index', index);
                    }
                }
                if(entry.label)
...
const wiredPanels = new WiredPanels({}, {
    activate(node) {
        ...
        function accept() {
            ...
            update.prevEncoding = backend.getPairOptionally(update.symbol, backend.symbolByName.Encoding);
            ...
            update.nextEncoding = backend.getPairOptionally(update.symbol, backend.symbolByName.Encoding);
            if(update.prevDataBytes !== update.nextDataBytes || update.prevDataLength !== update.nextDataLength) {
                if(update.prevEncoding != update.nextEncoding) {
                    if(update.prevEncoding != backend.symbolByName.Void)
                        setTripleVisibility(nodesToRemove, update.prevEncoding, update.panel, false);
                    if(update.nextEncoding != backend.symbolByName.Void)
                        setTripleVisibility(nodesToAdd, update.nextEncoding, update.panel, true);
                }
                const labelsToUpdate = new Set();
                for(const triple of backend.queryTriples(backend.queryMasks.VMM, [0, backend.symbolByName.Count, update.symbol]))
                    findLabelsOfComposite(triple[0], labelsToUpdate);
                wiredPanels.changeGraphUndoable(nodesToAdd, nodesToRemove, function(forward) {
                    if(update.prevEncoding != update.nextEncoding)
                        backend.getAndSetPairs(update.symbol, backend.symbolByName.Encoding, [forward ? update.nextEncoding : update.prevEncoding]);
    ...
    remove() {
        ...
        for(const node of wiredPanels.selection)
            switch(node.type) {
                        ...
                        case 'right': {
                            ...
                            if(wiredPanels.selection.has(oppositeSocket)) {
                                ...
                                if(triple[1] != undefined && triple[2] != undefined && node.orientation === 'left') {
                                    triples.add(triple);
                                    if(triple[1] === backend.symbolByName.Encoding)
                                        labelsToUpdate.add(triple[0]);
                                    findLabelsOfComposite(triple[0], labelsToUpdate);
                                }
                            } else {
                ...
                case 'panel':
                    const outerTriples = [
                        ...backend.queryTriples(backend.queryMasks.VMV, [0, node.symbol, 0]),
                        ...backend.queryTriples(backend.queryMasks.VVM, [0, 0, node.symbol])
                    ], update = {
                        'panel': node,
                        'symbol': node.symbol,
                        'dataBytes': backend.getRawData(node.symbol),
                        'dataLength': backend.getLength(node.symbol),
                        'triples': [...backend.queryTriples(backend.queryMasks.MVV, [node.symbol, 0, 0]), ...outerTriples]
                    };
    ...
    metaS(event) {
        ...
        Utils.downloadAsFile(string, 'Symatem.json');
    },
    metaO(event) {
        openFiles.click();
    },
